...
'''
ctx : 이미지 feature 의미.
ctx_dim: 512차원은 14x14개의 conv5 feature 중 1개 의미 =>
    - lstm_encoder를 사용하면 image feature -> hidden으로 embedding.
    ctx_dim도 options['dim'] * 2로 재정의.
    아마도 encoder output + encoder_rev output을 concat한 것.
...
def init_params(options):
    ...
    if options['lstm_encoder']: # potential feature that runs an LSTM over the annotation vectors
        ...
        params = get_layer('lstm')[0](options, params, prefix='encoder_rev',
                                      nin=options['ctx_dim'], dim=options['dim'])
        ctx_dim = options['dim'] * 2 # 이제 ctx_dim은 encoder/encoder_rev를 concat한 것?
...
# proj: [h, c, alpha, alpha_sample, ctx_, pstate_ ( dot(h[t-1], Wd_att)), pctx_, i, f, o, preact, alpha_pre, alpha[t-1]], pctx_list ( pctx_를 time에 따라 계속 append한 것)
    ...
    logit_shp = logit.shape # (n_timestep, n_samples, n_words)
    probs = tensor.nnet.softmax(logit.reshape([logit_shp[0]*logit_shp[1], logit_shp[2]]))

    # Index into the computed probability to give the log likelihood
    # x: n_timesteps x n_samples,
    x_flat = x.flatten() # x_flat: [1   27  398 2987 ...]
...
def train(dim_word=100,  # word vector dimensionality
    ...
    train, valid, test, worddict = load_data(load_train=True,
                                             load_dev=False,
                                             load_test=False,
                                             path='/home/taeksoo/Study/Multimodal/dataset/flickr30/')
